class LocationsController < ApplicationController
  def index
    ...
    @locations = @q.result(:distinct => true).includes(:connections, :meetings).page(params[:page]).per(10)
    @location_hash = Gmaps4rails.build_markers(@locations.where.not(:address_latitude => nil)) do |location, marker|
      ...
      marker.infowindow "<h5><a href='/locations/#{location.id}'>#{location.address}</a></h5><small>#{location.address_formatted_address}</small>"
    end
